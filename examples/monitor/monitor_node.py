...
class Node(BaseNode):
        ...
        if self.my_public_ip not in self.ip_to_telegram_mention:
            return [
                Message(
                    self.node_id,
                    payload={
                        "mention_on_telegram": self.mention_on_telegram,
    ...
    def completed_jobs(self, done_jobs: List[Job]):
        for job in done_jobs:
            logger.success(
                f"{job.name} job completed: payload={job.payload}, result={job.result}"
            )
        ...
        for job in assigned_jobs:
            if job.name == "ping":
                try:
                    ...
                    response = os.system(
                        "ping -c 1 " + job.payload["ip"] + "> /dev/null"
                    )
                    ping_result = not bool(response)
                except Exception:
                    job.set_result({"res": False, "ip": job.payload["ip"]})
                    self.failed_pings.append(
                        {"ip": job.payload["ip"], "date": datetime.utcnow().isoformat()}
                    )
                else:
                    if not ping_result:
                        self.failed_pings.append(
...
def run():
    ...
    node = Node(
        initial_config=Config(
            heartbeat_interval=HEARTBEAT_INTERVAL, max_assigned_jobs=1
        ),
