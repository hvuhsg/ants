...
class Job:
    ...
    def vector(self) -> float:
        date_sigmoid = 1 - sigmoid(
            round(self.created_at.timestamp())
        )  # older the better
        date_weight = 10
        id_sigmoid = sigmoid(self.assigned_to)  # tie break
        id_weight = 1
        return (
            (date_sigmoid * date_weight) + (id_sigmoid * id_weight) + self.status.value
        )
    ...
    def __hash__(self):
        return hash(
            f"{self.id}:{self.created_at}:{self.status}:{self.assigned_to}:{self.result}"
        )
