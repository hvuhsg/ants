...
class RequestHandler(BaseRequestHandler):
    def handle(self):
        ...
        self.server.communication.peers.add(
            (self.client_address[0], request_dict["port"])
        )
...
class SocketCommunication(BaseCommunication, Thread):
    ...
    def __init__(
        self,
        my_public_ip: str = "255.255.255.255",
        host="0.0.0.0",
        port=34687,
        pull_interval=10,
        ...
        self.socket_server_thread = Thread(
            target=self.socket_server.serve_forever, daemon=True
        )
    ...
    def _random_peer(self) -> tuple:
        ...
        for try_count in range(10):
            ...
            if random_peer in self.black_listed_peers:
                if (
                    time() - self.black_listed_peers[random_peer[0]]
                    >= BLACK_LIST_MAX_TIME
                ):
    ...
    def _update_ips(self):
        ips = self._request_from_random_peer(
            {"route": "ips", "port": self.server_address[1]}
        )
        if ips is None:
            return
        ips = map(tuple, ips)
    ...
    def _pull_state(self):
        state_dict = self._request_from_random_peer(
            {"route": "pull_state", "port": self.server_address[1]}
        )
